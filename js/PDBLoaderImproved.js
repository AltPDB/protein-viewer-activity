...
(function(THREE)
{
	...
	var maxDistance = 0;
	Object.keys(covalentRadius).forEach(function (key) {
		var radius = covalentRadius[key] = 0.01 * covalentRadius[key];
		if (radius > maxDistance) { maxDistance = radius; }
	});
	maxDistance = maxDistance * maxDistance * 4;
	...
	THREE.PDBLoader.prototype = {
		...
		createStickBallModels: function(molecule, options)
		{
			...
			options.bondFudgeFactor = options.bondFudgeFactor || 0.32;
			var bondFudgeFactor = options.bondFudgeFactor;
			...
			for (var i = 0; i < molecule.atoms.length; i++)
			{
				...
				*/
				...
				for(var j=i+1; j<molecule.atoms.length; j++)
				{
					...
					var neighborElement = neighbor.element.toLowerCase();
					var neighborCovalentRadius = covalentRadius[neighborElement];
					var covalentDist = atomCovalentRadius + neighborCovalentRadius;
					var covalentDistSquared = covalentDist * covalentDist;

					// they are bonded
					if( Math.abs(dist - covalentDistSquared) <= bondFudgeFactor*covalentDistSquared )
					{
						// add to bond map
						...
						// generate stick
						neighborPosition.set(neighbor.x, neighbor.y, neighbor.z);
						var start = mesh.position, end = neighborPosition;
